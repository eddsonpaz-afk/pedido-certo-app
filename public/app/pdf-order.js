(()=>{
  ...
  function showPdf(o){
    ...
    modal.innerHTML=`<div class="pdf-order-card"><div class="pdf-order-head"><div><h2>Seu pedido em PDF</h2><p>${o.id} • revise antes de enviar</p></div><button class="pdf-order-close" aria-label="Fechar">×</button></div><div class="pdf-order-preview">${activeUrl?`<iframe title="PDF do pedido" src="${activeUrl}#toolbar=0&navpanes=0"></iframe>`:'<div class="pdf-fallback">Não foi possível abrir a prévia do PDF neste navegador.<br>Você ainda pode enviar o resumo pelo WhatsApp.</div>'}</div><div class="pdf-order-actions">${activeUrl?`<a class="pdf-download" href="${activeUrl}" download="${o.id}.pdf">⬇ Baixar PDF</a>`:''}<button class="pdf-share" ${file?'':'disabled'}>↗ Compartilhar PDF</button><button class="pdf-whatsapp">WhatsApp • Enviar pedido</button><div class="pdf-order-hint">O WhatsApp não permite anexar um arquivo automaticamente pelo link. No celular, use “Compartilhar PDF” para enviar o arquivo diretamente ao WhatsApp; ou baixe o PDF e anexe na conversa.</div></div></div>`;
    document.body.appendChild(modal);
    modal.querySelector('.pdf-order-close').onclick=()=>{modal.remove()};
    ...
    modal.querySelector('.pdf-whatsapp').onclick=()=>{
      ...
      toast('Pedido salvo e WhatsApp aberto.');setTimeout(()=>location.reload(),700);
    };
  }
